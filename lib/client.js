...
export class TunnelVT {
  ...
  async _connectWS() {
    ...
    return new Promise((resolve, reject) => {
      ...
      sock.on("error", (err) => {
        if (sock.readyState !== WebSocket.OPEN) reject(err);
      });
      sock.on("close", () => {
        console.error("[tunnelvt] disconnected");
        resolve();
      });
    });
  }
      ...
      while (true) {
        try {
          ...
          if (e.message.includes("wrong password")) {
            console.error(`Invalid password for ${this._username}.`);
            this._password = await prompt("Password: ", true);
            if (!this._password) throw new Error("password required");
            continue;
          }

 ...
 * save to ~/.tunnelvt.json. Simple. No JWT, no device ID.
...
async function prompt(question, hidden) {
  const rl = readline.createInterface({ input: process.stdin, output: process.stdout });
  return new Promise((resolve) => {
    if (hidden) {
      // Simple hidden input via stdout manipulation
...
export class TunnelVT {
  /** @param {string} app @param {number} port */
  ...
  async _loadOrPrompt() {
    ...
    fs.writeFileSync(identityFile(), JSON.stringify({ username: this._username, password: this._password }));
  }
  ...
  _register() {
    return new Promise((resolve, reject) => {
      const onMsg = (data) => {
        ...
        if (ack.status === 201) console.error(`[tunnelvt] account created — welcome, ${this._username}!`);
        console.log(`https://gotunnel.vinstechid.com/${this._username}/${this.app}/`);
        console.error(`[tunnelvt] connected — ${this._username}/${this.app} -> localhost:${this.port}`);
        resolve();
      };
      this._ws.on("message", onMsg);
      this._send({
        type: "register", username: this._username, password: this._password,

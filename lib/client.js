...
function prompt(question, hidden) {
  return new Promise((resolve) => {
    ...
    if (!stdin.isTTY || !hidden) {
      // Fallback: readline (echoes on non-TTY; TTY but password shows)
      const rl = require("node:readline").createInterface({ input: stdin, output: stdout });
      rl.question("", (answer) => { rl.close(); resolve(answer.trim()); });
      return;
    }

    // TTY + hidden: raw mode, no echo.
    ...
    const onData = (buf) => {
      for (const ch of buf.toString("utf-8")) {
        ...
        if (ch === "\x7f" || ch === "\b") pass = pass.slice(0, -1);
        else if (ch >= " ") pass += ch;
      }
    };

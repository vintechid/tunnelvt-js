 ...
 *
 * Server URL is hardcoded. The end user only provides app, port, and token.
 *
...
export class TunnelVT {
  ...
  _register() {
    return new Promise((resolve, reject) => {
      ...
      this._send({ type: "register", token: this.token, device: this._device, app: this.app, port: this.port });
    });
  }
